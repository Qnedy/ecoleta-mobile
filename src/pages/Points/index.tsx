...
const Points = () => {
  ...
  useEffect(() => {
    api.get('/points', { params: { city: 'Palmas', uf: 'TO', items: [1, 2] } }).then(response => {
  ...
  return (
    <>
      ...
      <View style={styles.itemsContainer}>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: 36 }}>
          {items.map(item => (
            <TouchableOpacity key={String(item.id)} style={[ styles.item, selectedItems.includes(item.id) ? styles.selectedItem : {} ]} 
                              onPress={() => handleSelectItems(item.id)} activeOpacity={.6}>
